<template>
    <div
        ref="card"
        class="relative flex p-1 items-center bg-gray-100 dark:bg-gray-900 rounded-lg cursor-pointer"
        @click="clickStrategy"
    >
        <div class="h-8 w-8 mr-1 flex items-center justify-center">
        ...
        <Dropdown class="ml-auto" v-if="folder.id !== 'parent'">
            <template #trigger>
                <div
                    class="cursor-pointer flex items-center justify-center w-7 h-7 rounded-lg dark:hover:bg-gray-800"
                >
            ...
            <DropdownMenu @click.prevent.stop="showRename">
                <Icon type="pencil-alt" class="mr-2" width="16" height="16" />
                {{ __("Rename") }}
            </DropdownMenu>
